...
#else
/*
 * Remove probe points; later we will conver them to C calls that the
 * linker will be able to remove and put in a DOF segment for us
 * because of their magic names.
 */
#define DTRACE_CALL0ARGS(provider, name)
#define DTRACE_CALL1ARG(provider, name)
#define DTRACE_CALL2ARGS(provider, name)
