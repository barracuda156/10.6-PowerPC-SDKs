 ...
 * @abstract An instance of a OSMetaClass represents one C++ class
 * that the kernel's runtime type information system is aware of.
 */
...
/*! @function preModLoad
    ...
    @param kextID globally unique cString name of the kernel module being loaded. 
...
/*! @function modHasInstance
    ...
    @param kextID cString of the kernel module name.
...
/*! @function getKmodName
    @abstract Returns the bundle identifier
              of the kernel extension that defines the receiving metaclass.
    @result   Returns an OSSymbol representing the kext identifier.
    @discussion
    'Kmod' is the historical name for the executable of a kernel extension.
    Note that this method returns the bundle identifier of the kernel extension,
    and not the name of the bundle nor the filename of the executable.
 */
...
#define OSDefineMetaClassWithInit(className, superClassName, init)	\
    ...
    className ::MetaClass::MetaClass()					\
        : OSMetaClass(#className, className::superClass, (unsigned int) sizeof(className)) \
        { init; }
