...
@class NSCollectionView, NSMutableIndexSet;
@protocol NSCollectionViewDelegate;

@interface NSCollectionViewItem : NSViewController <NSCopying, NSCoding> {
@private
    NSCollectionView *_collectionView;
    struct {
	unsigned int isSelected:1;
	unsigned int suppressSelectionChangedNotification:1;
	unsigned int loadFromNib:1;
...
/* This method computes and returns an image to use for dragging. You can override this to return a custom drag image, or call it from the delegate method to get the default drag image. 'items' is an array of the NSCollectionViewItem instances being dragged. 'event' is a reference to the mouse down event that began the drag. 'dragImageOffset' is an in/out parameter. This method will be called with dragImageOffset set to NSZeroPoint, but it can be modified to re-position the returned image. A dragImageOffset of NSZeroPoint will cause the image to be centered under the mouse. By default, an image will be created that contains a rendering of the visible portions of the views for each item. If the equivalent delegate method is present, it will be preferred over this method.
 */
- (NSImage *)draggingImageForItems:(NSArray *)items withEvent:(NSEvent *)event offset:(NSPointPointer)dragImageOffset AVAILABLE_MAC_OS_X_VERSION_10_6_AND_LATER;
 ...
 */
- (BOOL)collectionView:(NSCollectionView *)collectionView canDragItems:(NSArray *)items withEvent:(NSEvent *)event AVAILABLE_MAC_OS_X_VERSION_10_6_AND_LATER;
 ...
 */
- (BOOL)collectionView:(NSCollectionView *)collectionView writeItems:(NSArray *)items toPasteboard:(NSPasteboard *)pasteboard AVAILABLE_MAC_OS_X_VERSION_10_6_AND_LATER;

/* The delegate can support file promise drags by adding NSFilesPromisePboardType to the pasteboard in -collectionView:writeItems:toPasteboard:. NSCollectionView implements -namesOfPromisedFilesDroppedAtDestination: to return the results of this delegate method. This method should return an array of filenames (not full paths) for the created files. The URL represents the drop location. For more information on file promise dragging, see documentation for the NSDraggingSource protocol and -namesOfPromisedFilesDroppedAtDestination:. You do not need to implement this method for your collection view to be a drag source.
 */
- (NSArray *)collectionView:(NSCollectionView *)collectionView namesOfPromisedFilesDroppedAtDestination:(NSURL *)dropURL forDraggedItems:(NSArray *)items AVAILABLE_MAC_OS_X_VERSION_10_6_AND_LATER;

/* Allows the delegate to construct a custom dragging image for the items being dragged. 'items' contains the NSCollectionViewItem instances being dragged. 'event' is a reference to the  mouse down event that began the drag. 'dragImageOffset' is an in/out parameter. This method will be called with dragImageOffset set to NSZeroPoint, but it can be modified to re-position the returned image. A dragImageOffset of NSZeroPoint will cause the image to be centered under the mouse. You can safely call -[NSCollectionView draggingImageForItems:withEvent:offset:] from within this method. You do not need to implement this meethod for your collection view to be a drag source.
 */
- (NSImage *)collectionView:(NSCollectionView *)collectionView draggingImageForItems:(NSArray *)items withEvent:(NSEvent *)event offset:(NSPointPointer)dragImageOffset AVAILABLE_MAC_OS_X_VERSION_10_6_AND_LATER;
...
/* This method is used by the collection view to determine a valid drop target. Based on the mouse position, the collection view will suggest a proposed index and drop operation. These values are in/out parameters and can be changed by the delegate to retarget the drop operation. The collection view will propose NSCollectionViewDropOn when the dragging location is closer to the middle of the item than either of its edges. Otherwise, it will propose NSCollectionViewDropBefore. You may override this default behavior by changing proposedDropOperation or proposedDropIndex. This method must return a value that indicates which dragging operation the data source will perform. It must return something other than NSDragOperationNone to accept the drop.
    ...
    Note: to receive drag messages, you must first call -[NSCollectionView registerForDraggedTypes:] for the drag types you want to support (typically this is done in -awakeFromNib). You must implement this method for your collection view to be a drag destination.
 */
