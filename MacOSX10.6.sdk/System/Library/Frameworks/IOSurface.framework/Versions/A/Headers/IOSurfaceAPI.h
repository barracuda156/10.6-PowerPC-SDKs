...
/* kIOSurfaceWidth  - CFNumber for the width of the IOSurface buffer in elements. */
extern const CFStringRef kIOSurfaceWidth					IOSFC_AVAILABLE_STARTING(__MAC_10_6, __IPHONE_NA);

/* kIOSurfaceHeight - CFNumber for the height of the IOSurface buffer in elements. */
extern const CFStringRef kIOSurfaceHeight					IOSFC_AVAILABLE_STARTING(__MAC_10_6, __IPHONE_NA);

/* kIOSurfaceBytesPerRow - CFNumber for the bytes per row of the buffer.   Defaults to an appropriately aligned value of BufferWidth * BytesPerElement. */
...
/* kIOSurfacePlaneWidth  - CFNumber for the width of this plane in elements.  Required for image planes. */
extern const CFStringRef kIOSurfacePlaneWidth					IOSFC_AVAILABLE_STARTING(__MAC_10_6, __IPHONE_NA);

/* kIOSurfacePlaneHeight  - CFNumber for the height of this plane in elements.  Required for image planes. */
extern const CFStringRef kIOSurfacePlaneHeight					IOSFC_AVAILABLE_STARTING(__MAC_10_6, __IPHONE_NA);

/* kIOSurfacePlaneBytesPerRow    - CFNumber for the bytes per row of this plane.  If not specified, defaults to an appropriately aligned
                                           value of PlaneWidth * PlaneBytesPerElement. */
...
/* This will return the last known seed value of the buffer as of the last lock or unlock information.
   It is a very fast call but is not currently guranteed to be up to date outside of a buffer lock.   
   For now if you need an accurate seed value, you should lock/unlock the buffer. */
...
/* These are similar to IOSurfaceLock()/IOSurfaceUnlock() except that it lets IOSurface
   know that you only intend to read or modify a single plane of the IOSurface buffer.  In particular, 
   a single plane write unlock will increment the global buffer modification seed and just the seed of
   the plane that was written to.   The other plane seeds will not be modified. */
IOReturn IOSurfaceLockPlane(IOSurfaceRef buffer, size_t planeIndex, uint32_t options, uint32_t *seed)
	IOSFC_AVAILABLE_STARTING(__MAC_10_6, __IPHONE_NA);

IOReturn IOSurfaceUnlockPlane(IOSurfaceRef buffer, size_t planeIndex, uint32_t options, uint32_t *seed)
	IOSFC_AVAILABLE_STARTING(__MAC_10_6, __IPHONE_NA);
...
uint32_t IOSurfaceGetSeedOfPlane(IOSurfaceRef buffer, size_t planeIndex)
	IOSFC_AVAILABLE_STARTING(__MAC_10_6, __IPHONE_NA);
...
/* The following are some "well-known" keys used to transport common values along with an IOSurface */

/* kIOSurfaceYCbCrMatrix defines the matrix transformation to be used to convert from Y,Cb,Cr values to
   RGB values.   The RGB color primaries are actually defined by a serialized CGColorSpaceRef that
   would normally be attached as a value using the kIOSurfaceColorSpace key (see below) */ 
extern const CFStringRef kIOSurfaceYCbCrMatrix;

/* These are the supported/well-known YCbCr matrix enums.  These are the same numeric values
   as specified for the Matrix in http://developer.apple.com/quicktime/icefloe/dispatch019.html. */
enum
{
	kIOSurfaceYCbCrMatrix_ITU_R_709_2   = 1,
	kIOSurfaceYCbCrMatrix_Unknown       = 2,
	kIOSurfaceYCbCrMatrix_ITU_R_601_4   = 6,    
	kIOSurfaceYCbCrMatrix_SMPTE_240M_1995 = 7
};
